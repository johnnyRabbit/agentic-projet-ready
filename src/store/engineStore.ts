...
export const useEngineStore = create<EngineState>((set, get) => ({
  ...
  initialize: async () => {
    ...
    contextEngine.addContext('proj-demo', {
      type: 'architecture',
      content:
        'Tech Stack: React Native + Expo, Node.js/NestJS backend, PostgreSQL, XState for state management, WebSocket for real-time updates',
    ...
    for (let i = 0; i < workflow.steps.length; i++) {
      ...
      try {
        ...
        if (run.status === 'complete') {
          addLog(
            `✅ ${step.name} — Complete (€${run.totalCost.toFixed(4)}, ${run.totalTokens} tokens)`
          );
        } else {
        ...
        if (run.output) {
          contextEngine.addContext(currentProjectId, {
            type:
              step.agentRole === 'developer'
                ? 'code'
                : step.agentRole === 'tester'
                  ? 'test'
                  : step.agentRole === 'reviewer'
                    ? 'decision'
                    : 'task',
            content: run.output,
  ...
  resolveApproval: (id, decision) => {
    ...
    set(() => ({ approvals: harness.getApprovalQueue() }));
  },
  ...
  loadState: async () => {
    try {
      const savedState = await loadState<Partial<EngineState>>('engine-state');
      if (savedState) {
        set({
        ...
        logger.debug('✅ State restored from IndexedDB');
      }
    } catch (error) {
