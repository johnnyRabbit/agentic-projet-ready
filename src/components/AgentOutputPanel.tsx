...
import { CheckCircle2, XCircle, Copy, ChevronDown, ChevronUp, Code, FileText, Shield, Brain } from 'lucide-react';
...
export function AgentOutputPanel({ runs }: AgentOutputPanelProps) {
  ...
  const getIcon = (role: string) => {
    switch (role) {
      case 'developer': return <Code size={14} className="text-indigo-400" />;
      case 'reviewer': return <Shield size={14} className="text-purple-400" />;
      case 'requirements': return <FileText size={14} className="text-blue-400" />;
      case 'planner': return <Brain size={14} className="text-green-400" />;
      default: return <Brain size={14} className="text-slate-400" />;
    }
  };

  const formatOutput = (output: string, role: string) => {
    try {
        ...
        {runs.slice().reverse().map(run => (
          <div key={run.id} className="border border-dark-500 rounded-lg overflow-hidden">
            ...
            >
              ...
              <span className="text-xs text-white font-medium capitalize flex-1">{run.agentRole}</span>
              <span className={`text-xs ${run.status === 'complete' ? 'text-success' : 'text-danger'}`}>
              ...
              {expandedRun === run.id ? <ChevronUp size={14} className="text-slate-400" /> : <ChevronDown size={14} className="text-slate-400" />}
            </button>
              ...
              <div className="bg-dark-900 p-4 border-t border-dark-500">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-slate-400">
                    Model: {run.modelCalls[0]?.model || '—'} | Tokens: {run.totalTokens} | Latency: {run.modelCalls[0]?.latency || 0}ms
                  </span>
                  <button
